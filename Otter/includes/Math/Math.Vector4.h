...
#pragma clang diagnostic pop
        ...
        Vector(const Vector<4, TNumber>& other)
            : X(other.X), Y(other.Y), Z(other.Z), W(other.W)
        {
        }

        Vector(Vector<4, TNumber>&& other) noexcept
            : X(other.X), Y(other.Y), Z(other.Z), W(other.W)
        {
        }

        Vector<4, TNumber>& operator=(const Vector<4, TNumber>& other)
        {
            for (UInt8 i = 0; i < 4; ++i)
        ...
        Vector<4, TNumber>& operator=(Vector<4, TNumber>&& other) noexcept
        {
            for (UInt8 i = 0; i < 4; ++i)
                Values[i] = other.Values[i];
        ...
        Vector<4, TNumber>& operator+=(const Vector<4, T>& other)
        {
            for (UInt8 i = 0; i < 4; ++i)
        ...
        Vector<4, TNumber>& operator+=(const Vector<4, TNumber>& other) noexcept
        {
            for (UInt8 i = 0; i < 4; ++i)
                Values[i] += other.Values[i];
        ...
        template<AnyNumber T>
        Vector<4, TNumber> operator+(const Vector<4, T>& other) const
        {
            return Vector<4, TNumber>(*this) += other;
        }
        ...
        bool operator==(const Vector<4, TOtherNumber>& other) const noexcept
        {
            ...
            return Math::AreApproximatelyEqual(X, other.X)
                   ...
                   && Math::AreApproximatelyEqual(W, other.W);
        }
    ...
    namespace Math
    {
        // TODO: Implement Vector4 functions
    }
}
